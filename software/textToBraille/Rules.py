...
class MatchRule(MapRule):
    ...
    def __call__(self, text: str, chargroups: dict[str, list[str]], variables: list[int]) -> tuple[str, list[int]]:
        if not self.match:
            ...
            self.to_match = tuple(re.sub(r"(?<=[^!]%)(\[.+]|.)",
                   lambda m: f"[{''.join(chargroups[self.char_attributes[m.group(0)]])}" if len(m.group(0)) == 1
                   else "[" + ''.join(''.join(chargroups[self.char_attributes[i]]) for i in m.group(0)[1:-1]) + "]",
                   m) for m in self.to_match)
            self.to_match = tuple(re.sub(r"!%(\[.+]|.)",
                   lambda m: f"[^{''.join(chargroups[self.char_attributes[m.group(0)]])}" if len(m.group(0)) == 1
                   else "[^" + ''.join(''.join(chargroups[self.char_attributes[i]]) for i in m.group(0)[1:-1]) + "]",
                   m) for m in self.to_match)

            self.match=re.compile("(?<=" + self.to_match[0] + ")" +
                                  self.to_match[1] +
                                  "(?=" + self.to_match[2] + ")")
